...
void check_history(struct account array[],FILE *fptr);
...
void check_history(struct account array[],FILE *fptr){
    ...
    for (int j = 0; j < i; j++) {
        if(account_verification == array[j].account_number) {
            ...
            while (fgets(detail, sizeof(detail), fptr) != NULL) {
                printf("%s", detail);
            }
            break;
        }
    ...
    if(verify==0){
    	printf("SORRY! Inccorect Account Number :");
	}
